...
export function AdjudicateModal({
  ...
  if (!user) {
    return (
      <Link
        ...
        className="mt-8 w-full md:w-auto md:px-12 py-4 bg-white text-bg font-black rounded-full text-sm tracking-widest uppercase inline-block text-center hover:bg-indigo transition"
      >
        Desbloquear
      </Link>
    );
  ...
  if (alreadyUnlocked && !done) {
    return (
      <Button variant="indigo" size="lg" disabled className="mt-8">
        ✓ Ya lo tienes
      </Button>
    );
  }
  ...
  if (done) {
    return (
      <>
        <div className="mt-8 p-6 bg-surface rounded-2xl border border-indigo/30">
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            ...
            {profile?.username && (
              <Link
                ...
                className="px-4 py-2 border border-white/20 rounded-full text-sm hover:border-indigo transition self-center"
              >
                Ver mi perfil
              </Link>
            )}
